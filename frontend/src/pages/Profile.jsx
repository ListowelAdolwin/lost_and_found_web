...
function Profile() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const [claimedRes, lostRes, foundRes, reportedRes] =
					await Promise.all([
						axios.get(
							`${BASE_URL}/api/claim/profile/${currentUser?._id}`
						),
						axios.get(
							`${BASE_URL}/api/lost-items/profile/${currentUser?._id}`
						),
						axios.get(
							`${BASE_URL}/api/found-items/profile/${currentUser?._id}`
						),
						axios.get(
							`${BASE_URL}/api/report-found/profile/${currentUser?._id}`
						),
					]);
	...
	return (
		<div className="min-h-screen">
			...
			<div className="mt-1 min-h-screen mb-20 dark:text-white">
				...
				>
					<h2 className="text-2xl font-semibold text-start px-4">
						Your Lost Items{" "}
					</h2>
					<p className="text-xs italic px-4 pb-3">
						Lost items you have posted
					</p>
					<div
						id="lost-items"
				...
				>
					...
					<p className="text-xs italic px-4 pb-3">
						Items you found and were good enough to post
					</p>
					<div
						id="found-items"
						className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-6 justify-items-center justify-center"
