...
const ItemClaims = () => {
	...
	const approveClaim = async () => {
		try {
			const res = await axios.post(
				`${BASE_URL}/api/claim/approve/${claimId}`,
				{ contact }
			);
			console.log(res);
			if (res.status === 200) {
	...
	if (loading) {
		return <p className="p-5 dark:text-white min-h-screen">Loading...</p>;
	}
	...
	return (
		<div className="container mx-auto p-4 min-h-screen">
			...
			{claims.length === 0 ? (
				<p className="text-center">No Claims yet for item</p>
			) : (
